    ...
    A: LinalgScalar + Send + Sync,
{
    ...
    fn divide_at(mut self, index: usize) -> (Self, Self) {
        ...
        if axis == 0 {
            ...
            let (br, _bc) = self.a.dim();
            let (dr, _dc) = self.d.dim();
            let ind_calc = ((((index - 1) as f32) / (row as f32)) + 1.0) as usize;
            let (b1, b2) =
                my_ndarray::divide_at_id_along_axis(self.b, ind_calc * dr / br, Axis(axis));
