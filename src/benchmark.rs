...
pub fn benchmark(height: usize, a: ArrayView<f32,Ix2>, b: ArrayView<f32,Ix2>, strategy: Policy) -> u64 {
...
pub fn benchmark_faster(height: usize, a: ArrayView<f32,Ix2>, b: ArrayView<f32,Ix2>, strategy: Policy) -> u64 {
            let mut dest = Array::zeros((height, height));
            let mut vec = Vec::new();
            vec.push(([(a,b)].to_vec(),dest.view_mut()));
            let mat = matrix::Matrix {
                            ...
                            .for_each(|e| {
                                for (vect,mut output) in e.matrix {
                                    ...
                                    for (a,b) in  vect {
                                        let dima = a.shape();
                                        let dimb = b.shape();
                                        if dima[0] == 0 || dima[1] == 0 || dimb[0] == 0 || dimb[1] == 0 ||rrow ==0 || rcol ==0 {
                                            continue;
                                        }
                                        ...
                                        let mut slicer = unsafe {from_raw_parts_mut(raw_ptr_r,len_r)};
                                          faster_vec::multiply_add(
                                                &mut slicer, &slicea, &sliceb, dima[1], dima[0], dimb[1], dimb[0], dimr[1], dimr[0], 
                                                    stridesa[0] as usize,stridesb[0] as usize,strides[0] as usize
                                            );
                                    }
                                }
                            });
...
pub fn benchmark_adaptive(height: usize, a: ArrayView<f32,Ix2>, b: ArrayView<f32,Ix2>, strategy: Policy) -> u64{
    let mut dest = Array::zeros((height, height));
