...
fn main() {
    ...
    let height = 4000;
    let width =  4000;
    let an = Array::from_shape_fn((height, width), |(i, j)| ((((j + i * width) % 3) as f32 + random) as f32));
    let bn = Array::from_shape_fn((width, height), |(i, j)| ((((j + 7 + i * height) % 3) as f32 - random) as f32));
    ...
    matrix_half.cut().with_policy(Policy::Join(4000*4000/64+1))
        .for_each(|e| {
            let (r,c)= e.d.dim();
            subgraph("prod",(r*c)^ 3,|| {
    ...
    }).1.save_svg("join_64_8threads.svg").expect("Saving failed");
}
